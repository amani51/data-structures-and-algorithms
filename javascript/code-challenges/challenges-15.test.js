...
const screenForNames = (arr) => {
  // Solution code here...
  return  arr.filter(item=> /^([Dr.]+\s)+[a-zA-Z]+$/.test(item)||/^([Mrs.]+\s)+[a-zA-Z]+$/.test(item))
}
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let newArr= arr.filter(item=>item.mass>77)
  return newArr.map(item=>item.name).toString().replace(",", " - ")
};
...
const detectTicTacToeWin = (board) => {
  ...
  let x1=[board[0][0],board[1][1],board[2][2]]
  let x2=[board[0][2],board[1][1],board[2][0]]
  let newArr=[row1,row2,row3,col1,col2,col3,x1,x2]

  let checkArr= newArr.map(item=>{
    if (item.every(item1=>item1=="O")){
    return true
    }else if (item.every(item1=>item1=="X")){
    return true
    }else {
      return false 
    }
