...
const getNames = (arr) => {
  // Solution code here...
 const newArr= arr.map((personName)=>{
    let stringArr= personName.name.split("")
    let reversedName=stringArr.reduce((acc,value) => [value].concat(acc));
   stringArr=reversedName.join('')
      return stringArr
    })
    return newArr
};
